...
public class GalleryController {
    ...
    public ResponseEntity<Spotting> getSpotting(@PathVariable Integer id) {
    ...
    public ResponseEntity<List<Spotting>> getByAirline(@PathVariable Integer airlineId) {
        return ResponseEntity.ok(galleryService.getByAirline(airlineId));
    }

    // By aircraft
    @GetMapping("/spotting/filter/aircraft/{aircraftId}")
    public ResponseEntity<List<Spotting>> getByAircraft(@PathVariable Integer aircraftId) {
        return ResponseEntity.ok(galleryService.getByAircraft(aircraftId));
    }

    // By spot location
    @GetMapping("/spotting/filter/airport/{airportId}")
    public ResponseEntity<List<Spotting>> getByAirport(@PathVariable Integer airportId) {
